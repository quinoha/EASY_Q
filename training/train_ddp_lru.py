...
def train_worker(rank: int, world_size: int, distance: int, p_start: float, p_target: float, total_steps: int, batch_size: int, lr: float):
    ...
    # --- Cosine LR Schedule (Paper: cosine decay over training) ---
    scheduler_muon = optim.lr_scheduler.CosineAnnealingLR(optimizer_muon, T_max=total_steps, eta_min=0)
    scheduler_adamw = optim.lr_scheduler.CosineAnnealingLR(optimizer_adamw, T_max=total_steps, eta_min=0)
    ...
    # --- Curriculum: Paper uses ≤2% total, but user tuned for 20K steps ---
